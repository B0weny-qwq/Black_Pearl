...
#define		AUTODRIVE_WORK_OVERTIME			10*60*100		// 10分钟
...
{	
	...
	if (autoDrive_State != AUTO_DRIVE_IDLE)		//当前已经在自动返航模式
		return FALSE;
	if ((point->jingdu_EW != 'E') && (point->jingdu_EW != 'W'))		// 返航点无效
		return FALSE;
	...
	if ((distance > 50) && (distance < 800))		// 距离大于50米  小于600米
		return TRUE;
	return FALSE;
}
...
void autoDrive_Set_ReturnPosition(uint8_t *data_m){
	...
	autodrive_work_overtime = AUTODRIVE_WORK_OVERTIME;		// 返航超时判断
	autoDrive_fail_flag = 0;
}
...
void autoDrive_Handle(void){
    ...
    switch (autoDrive_State)
    {
        ...
        case AUTO_DRIVE_RUNING:{
            ...
            if (autodrive_work_overtime)			// 返航超时 推出自动返航

...
#include "autoDrive.h"
uint8_t nmea41_Angel[10] = {0};
uint8_t nmea41_JingDu[12] = {0},nmea41_WeiDu[12] = {0};
uint8_t nmea41_GSV_NUM[3] = {0};
...
void nmea41_protocal_Resolve_Handle(uint8_t *data_m,uint16_t length){
    ...
    if(strstr((char *)cmdId,(char *)&"VTG")!=NULL){
        nmea41_Resolve_VTG(protocal,protocal_len);
    }else if(strstr((char *)cmdId,(char *)&"GGA")!=NULL){
       nmea41_Resolve_GGA(protocal,protocal_len);
    }else if(strstr((char *)cmdId,(char *)&"GSV")!=NULL){

...
int8_t qmc6309_get_chipid(void)
{
	...
	{
		QMC6309_LOG("qmc6309_get_chipid-ok slave:0x%bx chipid = 0x%bx\r\n", p_mag.slave_addr, chip_id);
		return 1;
	}
	else
	{
		QMC6309_LOG("qmc6309_get_chipid-fail slave:0x%bx chip_id = 0x%bx\r\n", p_mag.slave_addr, chip_id);
		return 0;
	}
}
...
int8_t qmc6309_enable(void)
{
	...
	{	ret = qmc6309_write_reg(QMC6309_CTL_REG_TWO, p_mag.ctrl2.value);
		...
		{	
			QMC6309_LOG("qmc6309_enable  succ!!!\r\n");
			break;
		}
		else
			QMC6309_LOG("qmc6309_enable fail !!!\r\n");
	}
	
	return ret;
}

int8_t qmc6309_disable(void)
{
	int8_t ret = 0;	
	...
	ctrl1.bit_reg.mode = QMC6309_MODE_SUSPEND;
...
{
	static unsigned char qmc6309_fail_num = 0;

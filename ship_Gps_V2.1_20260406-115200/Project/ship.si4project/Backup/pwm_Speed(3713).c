...
uint8_t nowPwmAccelerator_Get(void){
...
void Compass_Angel_adjust_Handle(void){
    ...
    if((DcMotor_PWM_Get_Direction() == DIRECTION_FORWARD) && (nowPwmAccelerator >= 10))
	{
		...
		{	
			...
			printf("C *** Gain=%bd A=%d\r\n",pwmTurnAdjust.speed_gain ,angel_abs);
			...
			{	if (angel_abs > 60)
				...
				{	
					...
					{	pwmTurnAdjust.speed_gain -= gain_angel;
						adjust_angel_offet = gain_angel - 2;
					}
					else
		...
		// printf("redirect==%bd,%bd\r\n",DcMotor_PWM_Get_Direction(),Pwm_Speed_Get_Turn_Direction());
		//startAveAngel = 0;

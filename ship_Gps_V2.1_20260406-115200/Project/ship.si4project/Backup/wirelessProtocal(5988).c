...
uint8_t idata cruise_calib_time;
...
void WirelessProtoca_Motor_Control(uint8_t leftRight,uint8_t frontBack){
    ...
    if(leftRight < 110 && leftRight > 90 && frontBack > 110){
        ...
        if(abs_frontBack > abs_leftRight && (abs_leftRight > 10 || abs_frontBack > 12)){
            if(frontBack > 110){
                DcMotor_Direction_Set(DIRECTION_FORWARD);

			#if (CRUISE_CORRECT_MODE == CRUISE_LEFT_CORRECT)
				if (cruise_calib_time > CORRECT_START_TIME)
					DcMotor_MRight_PWM_Set(TRUE,10);
				else
					DcMotor_MRight_PWM_Set(TRUE,(frontBack-100));
				
		         DcMotor_MLeft_PWM_Set(TRUE,frontBack-100);
				 
				//DcMotor_MLeft_PWM_Set(TRUE,frontBack-100  + PWM_DIFF_LEFT_TAB[(frontBack-100)/40]);
				//printf("pwm = %bd\r\n",frontBack-100  + PWM_DIFF_LEFT_TAB[(frontBack-100)/40]);
			#else
				DcMotor_MRight_PWM_Set(TRUE,frontBack-100);
				if (cruise_calib_time > CORRECT_START_TIME)
	            	DcMotor_MLeft_PWM_Set(TRUE,10);
				else
					DcMotor_MLeft_PWM_Set(TRUE,(frontBack-100);
				
			#endif
				if (cruise_calib_time > CORRECT_PERIOD_TIME)
				 {	 cruise_calib_time = 0;
					 printf("pwm loop!!!\r\n");
				 }
                //DcMotor_MRight_PWM_Set(TRUE,frontBack-100);
        ...
        }else if(abs_frontBack < abs_leftRight && (abs_leftRight > 10 || abs_frontBack > 12)){
            if(leftRight <= 90 ){
                DcMotor_Direction_Set(DIRECTION_LEFT);
                DcMotor_MRight_PWM_Set(TRUE,100-leftRight);
                DcMotor_MLeft_PWM_Set(TRUE,100-leftRight);
            ...
            if(Get_DcMotor_PWM_CruiseControl_Mode()!= CRUISE_STOP){
                ...
                switch(myDcMotor_PWM_CruiseControl_Mode){
                    case CRUISE_LOW:
					#if (CRUISE_CORRECT_MODE == CRUISE_NO_CORRECT)
		                DcMotor_MLeft_PWM_Set(TRUE,30);
		                DcMotor_MRight_PWM_Set(TRUE,30);
		            #elif (CRUISE_CORRECT_MODE == CRUISE_LEFT_CORRECT)
		                DcMotor_MLeft_PWM_Set(TRUE,30);
						if (cruise_calib_time > CORRECT_START_TIME)
							DcMotor_MRight_PWM_Set(TRUE,0);
						else
		                	DcMotor_MRight_PWM_Set(TRUE,30);
		            #elif (CRUISE_CORRECT_MODE == CRUISE_RIGHT_CORRECT)
						if (cruise_calib_time > CORRECT_START_TIME)

...
void autoDrive_active(void){
	...
	{	
		...
		if ((autodrv_cfg.ret_point.jingdu_EW != 'E') && (autodrv_cfg.ret_point.jingdu_EW != 'W'))		// 返航点无效
			return;
		if (autodrv_cfg.ret_point.jingdu_Left == 0)
			return ;
...
{		
	...
	memcpy(&debug_data_buf[index],&now_Gps_Position.jingdu_Right,4);
	index += 4;
	...
	memcpy(&debug_data_buf[index],&returnPosition.jingdu_Right,4);
	index += 2;
	
	memcpy(&debug_data_buf[index],&returnPosition.weidu_Left,2);
	index += 2;
	
	memcpy(&debug_data_buf[index],&returnPosition.weidu_Right,4);
	index += 2;
	...
	memcpy(&debug_data_buf[index],destination_Direction,2);
	index += 2;

	memcpy(&debug_data_buf[index],nowRun_Direction,2);
	index += 2;

	//RF_Send_Gps_Data(wirelessConfig.RF_Channel[2],LT8920_SEND);
	Wireless_Send(wirelessConfig.RF_Channel[2],0x17,(uint8_t *)debug_data_buf,index,LT8920_SEND);
}

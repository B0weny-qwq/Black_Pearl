    ...
    if(leftRight < 110 && leftRight > 90 && frontBack > 110){
        ...
        if(abs_frontBack > abs_leftRight && (abs_leftRight > 10 || abs_frontBack > 10)){
            if(frontBack > 110){
                ...
                if(Get_DcMotor_PWM_CruiseControl_Mode() != CRUISE_STOP)		// 巡航模式下 油门后退 退出巡航模式
                {	
                	if (frontBack < 30)
						Set_DcMotor_PWM_CruiseControl_Mode(CRUISE_STOP);
                }
...
void WirelessProtoca_Key_Control(uint8_t accelerator_leftRight,uint8_t accelerator_frontBack,uint8_t keyCode){
    ...
    if(lastKeyCode != keyCode){
        ...
        switch(lastKeyCode){
            ...
            break;
            case KEY_E_FLAG:
                if(accelerator_frontBack > 150){		// 前进模式下 按下E键 启动巡航模式
...
{	
	...
	uint8_t wirelessSendData[40]={0};  
	uint8_t index = 0;
	wirelessSendData[index++] = 0xaa;
	wirelessSendData[index++] = 2 + len;
	wirelessSendData[index++] = 0x17;
	...
	wirelessSendData[index++] = cal_checksum_XOR(&wirelessSendData[1],2 + len);
	wirelessSendData[index++] = 0xbb;
	//Wireless_Send(wirelessConfig.RF_Channel[2],0x17,(uint8_t *)wirelessSendData,index,LT8920_SEND);
	LT8920_TxData(wirelessConfig.RF_Channel[2],(uint8_t *)&wirelessSendData,index,LT8920_SEND);
}

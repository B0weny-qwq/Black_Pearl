...
WIRELESS_CONFIG idata wirelessConfig;
uint16_t idata wirelessAccelerator_OutTime_Times = 0;
uint16_t idata wirelessAccelerator_close_motor_time = 0;
uint16_t idata lowpower_check_Times = 0;

uint16_t pair_wait_rsp_time = 0;


bit is_forward_run = 0;
bit is_in_cruise_run = 0;
bit is_in_first_cruise_run = 0;
...
void WirelessProtocal_Receive_Handle(uint8_t *data_m,uint8_t len){
    ...
    bit protocalRecFinishFlag = 0;
    bit protocalCheckResult = 1;
    uint8_t protocalCheckSum = 0;
    // printf("wirlepro receive\r\n");
    // uprintf_array(data_m,len);

	if(len > WIRELESS_PROTOCAL_MAX_LEN){
        //printf("protocal len error %bd!!\r\n" ,len);
		//uprintf_array(data_m,len);
		//return;
		len = 10;
    }
    ...
    for(i = 0;i < len;i ++){
        ...
        switch(protocalIndex){
            ...
            case 1:{
				...
				if(protocalRecLen > (WIRELESS_PROTOCAL_MAX_LEN-2)){
                    protocalIndex = 0;
        ...
        if(protocalCheckResult){
			if (protocalIndex < WIRELESS_PROTOCAL_MAX_LEN)
            ...
            if(protocalRecFinishFlag){
                protocalRecFinishFlag = 0;
                ...
                if(protocalCheckSum == protocalData[protocalIndex-2] && protocalData[protocalIndex-1] == 0xbb){
                    ...
                    // uprintf_array(data_m,len);
                    WirelessProtocal_Resolve_Handle((uint8_t *)&protocalData);
                }
            }
        }
...
void WirelessProtoca_Motor_Control(uint8_t leftRight,uint8_t frontBack){
    ...
    abs_frontBack = abs(frontBack - 100) + 5;
    if(abs_frontBack > abs_leftRight && (abs_leftRight > 10 || abs_frontBack > 20))
		{
        if(frontBack > 110)
				{
						if(is_in_first_cruise_run)
								return ;
						if (!is_forward_run)
							comPass_cache_init_angel();
			
            DcMotor_Direction_Set(DIRECTION_FORWARD);
            nowPwmAccelerator_Set(frontBack-100);

            // printf("adjust acc1=%bd,%bd\r\n",speedLevel,adjustAccelerator);
		...
		else if(abs_frontBack < abs_leftRight && (abs_leftRight > 10 || abs_frontBack > 20))
		{
        if(leftRight <= 90 )
				{
		...
		{		// 调整方向�?继续巡航
        cruise_mode = Get_DcMotor_PWM_CruiseControl_Mode();
        if(cruise_mode != CRUISE_STOP)
				{			
						if(!is_in_cruise_run)
						{	
								if (!is_in_first_cruise_run)
								...
								DcMotor_PWM_CruiseControl_Reset_Handle();
						}
						is_in_first_cruise_run = 0;
						is_in_cruise_run = 1;
				}
				else
				{	
...
void WirelessProtoca_Key_Control(uint8_t accelerator_frontBack,uint8_t keyCode){
...
void WirelessProtocal_Resolve_Handle(uint8_t *data_m){
...
void WirelessProtocal_Accelerator_Resolve(uint8_t accelerator_leftRight,uint8_t accelerator_frontBack,uint8_t key){
    ...
    WirelessProtoca_Key_Control(accelerator_frontBack,key);
    WirelessProtocal_Accelerator_OutTime_Clear();
}


extern uint8_t idata nowPwmAccelerator;

void LowPower_back_check(void)
{	
	lowpower_check_Times ++;
	if (lowpower_check_Times > 600)
	{	
		lowpower_check_Times = 0;
		
		if (Power_ADC_Get_Level() == POWER_LEVEL_0)
		{
			if((autoDrive_Get_Mode() == AUTO_DRIVE_CLOSE) && (nowPwmAccelerator < 10))
			{	
				autoDrive_active();
			}
		}
...
void WirelessProtocal_Accelerator_OutTime_Handle(void){
    ...
    if(wirelessAccelerator_OutTime_Times < 30*100){
        ...
        DcMotor_MLeft_PWM_Set(FALSE,0);
		#endif
		printf("enter auto drive 0x%x!!!\r", wirelessAccelerator_OutTime_Times);
		autoDrive_active();
	...
	{	
		...
		{	
			...
			RF_key_chn_reinit();
			printf("RF CMD OVERTIME  REBOOT!!!\r");
...
void WirelessProtocal_Accelerator_OutTime_Clear(void){
    wirelessAccelerator_OutTime_Times = 0;
	wirelessAccelerator_close_motor_time = 300;		// 5秒收不到数据 如果电机在工作，停止工作
}


uint8_t RF_Pair_Get_Channel(uint8_t *data_m){
    uint8_t channel = 0;
...
uint8_t RF_Pair_Get_Key(uint8_t *data_m){
...
void RF_key_format(uint8_t *pairdata)
{	
	...
	RF_Pair_Get_Key((uint8_t *)&pairdata[0]);
    RF_Pair_Get_Channel((uint8_t *)&pairdata[0]);
}
...
void RF_Pair(uint8_t FreqChannel,uint8_t role){
	...
	printf("RF_Pair tx channel=%b02x\r\n",wirelessConfig.RF_Channel[0]);
}

void RF_key_chn_reinit(void)
{	
	uint8_t pairData[4]={0,0,0,0};

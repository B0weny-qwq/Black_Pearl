...
#ifdef		BOARD_12V
#define			RIGHT_DEFAULT_GAIN			30
#define			LEFT_DEFAULT_GAIN			30
#define			SPEED_GAIN_MIN				24

uint8_t code ANGEL_OFFSET_TAB[] = {3,4,6,7,8};

#elif   defined(MID_SHIP)
#define			RIGHT_DEFAULT_GAIN			27
#define			LEFT_DEFAULT_GAIN			29
...
void Compass_Angel_adjust_Handle(void){
    ...
    if((lastDirection == DIRECTION_FORWARD) && (nowPwmAccelerator >= 10))
	{
		...
		s_angel_first_check = 1;		// origin 0, modified on 2025-2-17
...
//			printf("C *** T=%d,a=%d\r\n",AdjustAngelTimes,angel_abs);	// 2025-1-17
//			printf("C *** Gain=%bd \r\n",pwmTurnAdjust.speed_gain);
...
//					printf("C *** CORRECT SPEED = %bd\r\n",correct_speed_gain);
			...
			else
			{
				...
				{	
					if ((correct_speed_gain <= SPEED_GAIN_MIN) || (correct_speed_gain >= LEFT_DIVIDE_MAX))
...
//			printf("C ### na=%d,la=%d\r\n",angel_abs,last_angel_offset);	// 2025-1-17
		}

...
PWMx_Duty PWMA_Duty;
uint8_t idata dcMotor_PWM_CruiseControl_Mode = CRUISE_STOP;

uint8_t idata lastDirection = 255;
...
void DCMotor_Pwm_Init(void){
	...
	PWMx_InitStructure.PWM_EnoSelect   = ENO4P|ENO4N;	//输出通道选择,	ENO1P,ENO1N,ENO3P,ENO3N,ENO3P,ENO3N,ENO4P,ENO4N / ENO5P,ENO6P,ENO7P,ENO8P
	PWM_Configuration(PWM4, &PWMx_InitStructure);			//初�?�化PWM,  PWMA,PWMB
...
void DcMotor_MRight_PWM_Set(uint8_t onoff,uint8_t accelerator){
    // P12 = onoff;
//	uint16 target_duty;											// add 20260204
//	
//	target_duty = 1200+15*accelerator;			// add 20260204
    if(onoff){
//			/*	chang 20260204 start*/
//			if(PWMA_Duty.PWM3_Duty < target_duty)
//			{
//				PWMA_Duty.PWM3_Duty +=DUTY_STEP;
//			}
//			if(PWMA_Duty.PWM3_Duty > target_duty)
//				PWMA_Duty.PWM3_Duty = target_duty;
//			else
//			{
//				PWMA_Duty.PWM3_Duty = target_duty;
//			}
        PWMA_Duty.PWM3_Duty = 1200+15*accelerator;
			/*	chang 20260204 stop*/
    }else{
...
//		target_duty = 1200+15*accelerator;			// add 20260204
    if(onoff){
//			/*	chang 20260204 start*/
//			if(PWMA_Duty.PWM1_Duty < target_duty)
//			{
//				PWMA_Duty.PWM1_Duty +=DUTY_STEP;
//			}
//			else
//			{
//				PWMA_Duty.PWM1_Duty = target_duty;
//			}
//			if(PWMA_Duty.PWM1_Duty > target_duty)
//				PWMA_Duty.PWM1_Duty = target_duty;
        PWMA_Duty.PWM1_Duty =  1200+15*accelerator;
			/*	chang 20260204 stop*/
    }else{
        PWMA_Duty.PWM1_Duty = 0;
    }
...
void DcMotor_PWM_CruiseControl_Set_Handle(void){
    ...
    switch (dcMotor_PWM_CruiseControl_Mode)
    {
        ...
        case CRUISE_LOW:{
            nowPwmAccelerator_Set(60);
			comPass_cache_init_angel();
        }
        break;

        case CRUISE_MIDDLE:{
            nowPwmAccelerator_Set(80);
			//comPass_cache_init_angel();
        }
        break;
...
void DcMotor_PWM_CruiseControl_Reset_Handle(void){
    DcMotor_Direction_Set(DIRECTION_FORWARD);
    switch (dcMotor_PWM_CruiseControl_Mode)
    {
        case CRUISE_LOW:{
...
#if 0
uint8_t DcMotor_PWM_Get_Direction(void){

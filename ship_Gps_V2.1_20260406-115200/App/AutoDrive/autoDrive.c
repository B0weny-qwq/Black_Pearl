...
uint8_t idata autoDrive_Switch = FALSE;

uint8_t idata autoDrive_State = AUTO_DRIVE_IDLE;
uint8_t idata autoDrive_Mode = AUTO_DRIVE_CLOSE;

uint8_t idata autoDrive_GetDirection_Times = 0;
uint16_t idata autoDrive_Get_Start_Direction_Turn_Times = 0;
uint16_t idata autodrive_work_overtime;
uint8_t idata autoDrive_fail_flag = 0;

GPS_POSITION idle_Gps_Position;
GPS_POSITION now_Gps_Position,last_Gps_Position;

GPS_POSITION  idata returnPosition;
GPS_POSITION  idata fishPosition;
...
RETURN_CONFIG_T idata autodrv_cfg;
...
//	printf("init jingDu=%u.%u,",autodrv_cfg.ret_point.jingdu_Left,autodrv_cfg.ret_point.jingdu_Right);
//    printf("init weidu=%u.%u,",autodrv_cfg.ret_point.weidu_Left,autodrv_cfg.ret_point.weidu_Right);
...
{	
	...
	if ((distance > 10) && (distance < 800))		// ¾àÀë´óÓÚ50Ã×  Ð¡ÓÚ600Ã×
	{	
//		printf("autodrive active!!\r\n");
		return TRUE;
	}
//	printf("autodrive active err! distance = %d \r\n",distance);
	return FALSE;
}
...
//    printf("fish jingDu=%u.%u,",fishPosition.jingdu_Left,fishPosition.jingdu_Right);
//    printf("fish weidu=%u.%u,",fishPosition.weidu_Left,fishPosition.weidu_Right);
...
void autoDrive_start_run_line(uint8_t speed)
{	
	uint8_t motor_dir = lastDirection;
	
	DcMotor_Direction_Set(DIRECTION_FORWARD);
	nowPwmAccelerator_Set(speed);
	
	//comPass_cache_init_angel();
	if (motor_dir != DIRECTION_FORWARD)
		comPass_cache_init_angel_delay();
}
...
void autoDrive_Handle(void){
    ...
    uint8_t direction_Sub_Abs = 0;
		uint16_t direction_Threshold = 0;
		int16_t direction_Diff = 0;
        ...
        case AUTO_DRIVE_START:{
            ...
            if(autoDrive_Get_Mode()==AUTO_DRIVE_GO_FISISH_POSITION){
                ...
                destination_Direction = autoDrive_Get_Direction_NowPosition_Destination((uint8_t *)&idle_Gps_Position,(uint8_t *)&returnPosition);
								...
								switch (destination_Direction)
								{
									case POSITION_NORTH:
									{
										direction_Diff = 0-nowAveAngel;
									}
									break;
   
									case POSITION_EAST_NORTH:
									{
										direction_Diff = 3150-nowAveAngel;
									}
									break;
									case POSITION_EAST:
									{
										direction_Diff = 2700-nowAveAngel;
									}
									break;
									case POSITION_EAST_SOUTH:
									{
										direction_Diff = 2250-nowAveAngel;
									}
									break;
									case POSITION_SOUTH:
									{
										direction_Diff = 1800-nowAveAngel;
									}
									break;
									case POSITION_WEST_SOUTH:
									{
										direction_Diff = 1350-nowAveAngel;
									}
									break;
									case POSITION_WEST:
									{
										direction_Diff = 900-nowAveAngel;
									}
									break;
									case POSITION_WEST_NORTH:
									{
										direction_Diff = 450-nowAveAngel;
									}
									break;
								}
													
//								direction_Diff = (destination_Direction-1)*450-nowAveAngel;
								direction_Diff = direction_Diff>0?direction_Diff:(direction_Diff + 3600);
								direction_Diff = direction_Diff>1800?(direction_Diff-3600):direction_Diff;
								if(direction_Diff > 50)
									DcMotor_Direction_Set(DIRECTION_LEFT);
								else if(direction_Diff < -50)
                ...
                autoDrive_Set_Turn_Times(abs(direction_Diff)>>3);//225-good	
        ...
        case AUTO_DRIVE_RUNING:{
            ...
            if (GPS_Position_Get_Position_Refresh_Flag()==TRUE && autoDrive_Get_Start_Direction_Turn_Times == 0)
            {
                ...
                if(autoDrive_Get_Mode()==AUTO_DRIVE_GO_FISISH_POSITION){
									...
									destination_Direction = autoDrive_Get_Direction_NowPosition_Destination((uint8_t *)&now_Gps_Position,(uint8_t *)&fishPosition);
                  ...
                  // printf("fish angel==%hu\r\n",destination_Angel);
									...
									destination_Direction = autoDrive_Get_Direction_NowPosition_Destination((uint8_t *)&now_Gps_Position,(uint8_t *)&returnPosition);
									if(destination_Angel == 65535){
                     return;
									}
								 //printf("back2 jingDu=%u.%u\r\n",returnPosition.jingdu_Left,returnPosition.jingdu_Right);
                 ...
                 if(destinationDistance < 1.5){
									...
									autoDrive_start_run_line(100);
									printf("Distance <1.5,go straight \r\n");
									return;
                 }
                 nowRun_Angel = nmea41_Get_Angel();
								 #ifdef		GPS_DK2_RMC
								 nmea41_print_Angel_Time();
								 #endif 
                    // if(1){
                  ...
                  printf("cal now angel=%hu\r",nowRun_Angel);
                    ...
                    if((nowRun_Angel>270 && destination_Angel < 90)){
                        ...
                        if(nowRun_Angel > destination_Angel){
                            ...
                            DcMotor_MRight_PWM_Set(TRUE,100);
													autoDrive_Set_Turn_Times(angel_Sub_Abs);
//                            autoDrive_Set_Turn_Times(angel_Sub_Abs*2);
                            printf("turn left=%hu\r\n",angel_Sub_Abs);
                        }else{
                            DcMotor_Direction_Set(DIRECTION_RIGHT);
                            DcMotor_MLeft_PWM_Set(TRUE,100);
                            DcMotor_MRight_PWM_Set(TRUE,100);
														autoDrive_Set_Turn_Times((angel_Sub_Abs));
//                            autoDrive_Set_Turn_Times((angel_Sub_Abs*3)/2);
                            printf("turn right=%hu\r\n",angel_Sub_Abs);
                        }
                    }
										*/
										
								 direction_Diff = (destination_Angel - nowRun_Angel + 360) % 360;
								 if (direction_Diff > 180) {
									turn_angle = 360 - direction_Diff;
                  turn_dir = DIRECTION_LEFT;
								 } else {
									turn_angle = direction_Diff;
                  turn_dir = DIRECTION_RIGHT;
								 }
...
//											return;
                    ...
                    // 
                  DcMotor_Direction_Set(turn_dir);
                  DcMotor_MLeft_PWM_Set(TRUE, 100);
                  DcMotor_MRight_PWM_Set(TRUE, 100);
                  autoDrive_Set_Turn_Times(turn_angle);  // 

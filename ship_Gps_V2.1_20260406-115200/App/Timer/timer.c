...
uint8_t idata SysTimer_10Ms_Flag = 0,SysTimer_1Ms_Times = 0;
uint8_t idata SysTimer_10ms_tick;
...
void SysTimer_delay10ms(uint8_t delay_cnt){
	...
	SysTimer_10ms_tick = delay_cnt;
	while(SysTimer_10ms_tick)
	{	
		if (SysTimer_10Ms_Flag)
		{	
			SysTimer_10Ms_Flag = 0;
			SysTimer_10ms_tick --;
		}
	}
}
...
uint8_t idata delay_ms_tick;

void SysTimer_delayms(uint8_t delay_cnt){
		
	delay_ms_tick = SysTimer_1Ms_Times;
	SysTimer_10ms_tick = delay_cnt;

	while(SysTimer_10ms_tick)
	{	
		if (delay_ms_tick != SysTimer_1Ms_Times)
		{	
			delay_ms_tick = SysTimer_1Ms_Times;
			SysTimer_10ms_tick --;
		}

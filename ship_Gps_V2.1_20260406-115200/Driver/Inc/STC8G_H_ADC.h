...
#define 	ADC_PowerOn(n)			ADC_CONTR = (ADC_CONTR & ~0x80) | (n << 7)	/* ADC??????? */
#define 	ADC_Justify(n)			ADCCFG = (ADCCFG & ~(1<<5)) | (n << 5)			/* ADC?????????????, ?????/????? */

//========================================================================
//                              ????????
//========================================================================

#define ADC_RES_12BIT	1		//0: MCU?10¦ËADC; 1: MCU?12¦ËADC
...
#define ADC_POWER	(1<<7)	//ADC ???
#define ADC_START	(1<<6)	//ADC ???????????¦Ë???????0
#define ADC_FLAG	(1<<5)	//ADC ??????????¦Ë????????0
#define ADC_EPWMT	(1<<4)	//??? PWM ??????? ADC ????
#define ADC_CH0		0
#define ADC_CH1		1
#define ADC_CH2		2
#define ADC_CH3		3
...
typedef struct
{
	u8	ADC_SMPduty;		//ADC ???????????????, 0~31????? SMPDUTY ???????????§³?? 10??
	u8	ADC_Speed;			//???? ADC ??????????	ADC_SPEED_2X1T~ADC_SPEED_2X16T
	u8	ADC_AdjResult;	//ADC???????,	ADC_LEFT_JUSTIFIED,ADC_RIGHT_JUSTIFIED
	u8	ADC_CsSetup;		//ADC ???????????? 0(???),1
	u8	ADC_CsHold;			//ADC ????????????? 0,1(???),2,3
} ADC_InitTypeDef;

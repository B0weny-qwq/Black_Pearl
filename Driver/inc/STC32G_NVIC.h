/**
 * @file    STC32G_NVIC.h
 * @brief   STC32G 中断开关与优先级控制宏
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件提供 Timer、EXTI、ADC、SPI、RTC、UART、I2C、LIN 等中断使能宏，
 * 以及部分优先级控制定义。当前工程的 `SYS_Init()` 和各外设模块会通过这些宏
 * 打开所需中断，但具体中断函数逻辑分布在 `Driver/isr/` 中。
 *
 * @note 当前职责边界：
 * - 只控制中断开关与优先级寄存器；
 * - 不承载中断业务逻辑，也不定义主循环调度策略。
 */

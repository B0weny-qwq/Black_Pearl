/**
 * @file    STC32G_UART.h
 * @brief   STC32G UART 驱动接口与缓冲区定义
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件定义 UART1~UART4 的驱动配置、发送接收缓冲区、状态结构体和基础 API。
 * 当前工程中最关键的两条链路是：
 * - UART1：供 `Log` 模块输出诊断文本；
 * - UART2：供 `GPS` 模块经 `RX2_Buffer` 接收 NMEA0183 字节流。
 *
 * @note 当前职责边界：
 * - Driver 层只维护串口寄存器配置、环形缓冲和中断读写；
 * - `GPS.c`、`Log.c`、官方 `App/` 示例等上层模块只消费这些缓冲与接口；
 * - 业务协议解析、NMEA 状态机和控制策略不在 Driver 层实现。
 */

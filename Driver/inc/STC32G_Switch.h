 ...
 * @brief   STC32G 外设引脚重映射宏定义
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件集中定义 UART、I2C、SPI、PWM、CAN、LIN 等外设的引脚切换宏与可选路由。
 * 当前工程中最关键的路由是：
 * - `UART2_SW(UART2_SW_P10_P11)`：GPS 使用 UART2 的 `P1.0/P1.1`
 * - `UART1`：日志链路使用默认/既定路由
 * - `I2C_SW(...)`：由传感器初始化链路决定磁力计/IMU 实际挂接引脚
 *
 * @note 当前职责边界：
 * - 本文件只定义寄存器级路由切换宏；
 * - 不校验资源冲突，冲突检查由上层初始化和文档约束负责；
 * - 改动路由前必须同步检查 `GPS`、`Log`、传感器和电机相关模块。
 */

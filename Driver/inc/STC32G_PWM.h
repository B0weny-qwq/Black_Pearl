/**
 * @file    STC32G_PWM.h
 * @brief   STC32G PWM 外设配置接口
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件定义 PWMA/PWMB 的输出使能、引脚路由、工作模式和初始化相关宏。
 * 当前工程的电机输出最终依赖底层 PWM 能力，但具体左右电机控制目标与混控策略
 * 由 `Code_boweny/Device/Motor` 与 `ShipControl` 负责。
 *
 * @note 当前职责边界：
 * - Driver PWM 只提供硬件波形输出能力；
 * - 不负责任何航向保持、差速混控或目标速度决策；
 * - 调整 PWM 引脚与频率时需同步检查电机端口映射。
 */

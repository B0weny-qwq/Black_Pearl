/**
 * @file    STC32G_Timer.h
 * @brief   STC32G Timer0~Timer4 配置接口
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件定义 Timer0~Timer4 的时钟源、运行控制、装载值与初始化结构体。
 * 当前工程中 Timer0 参与系统节拍，Timer2 与 UART2/GPS 波特率链路强相关，
 * 因此所有定时器复用都必须先检查现有资源分配。
 *
 * @note 当前职责边界：
 * - 只提供硬件定时器配置能力；
 * - 具体周期语义由 `User/`、`GPS`、`Motor` 等上层模块定义；
 * - 修改 Timer2 用途前必须重新核对 GPS 串口链路。
 */

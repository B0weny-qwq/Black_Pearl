/**
 * @file    STC32G_I2C.h
 * @brief   STC32G 硬件 I2C 驱动接口
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本头文件定义 STC32G 硬件 I2C 主从模式寄存器宏、初始化结构体和基础读写 API。
 * 在当前工程中，QMI8658 与 QMC6309 等传感器链路依赖 I2C 总线，但具体设备协议、
 * 重试策略和校验逻辑由各自设备模块实现，不在 Driver 层处理。
 *
 * @note 当前职责边界：
 * - Driver I2C 只提供底层总线收发能力；
 * - 设备地址、寄存器意义、姿态解算与校准逻辑都在上层模块；
 * - 若修改 I2C 引脚或速率，需要同步验证 IMU/磁力计初始化链路。
 */

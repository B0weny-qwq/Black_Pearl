/**
 * @file    STC32G_UART.c
 * @brief   STC32G UART 驱动实现
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本文件负责 UART1~UART4 的寄存器配置、发送缓冲推进和接收缓冲状态初始化。
 * 当前工程最关键的运行关系如下：
 * - `SYS_Init()` / 各上层模块调用 `UART_Configuration()` 初始化串口；
 * - `Driver/isr/STC32G_UART_Isr.c` 将收到的字节写入 `RXn_Buffer`；
 * - `GPS.c` 轮询消费 `RX2_Buffer`，`Log.c` 通过 UART1 发送调试文本。
 *
 * @note 当前职责边界：
 * - 本文件只提供串口驱动能力和缓冲管理；
 * - 不直接解析 GPS、无线协议或控制命令；
 * - UART2 的具体引脚选择由上层通过 `UART2_SW(...)` 先完成。
 */

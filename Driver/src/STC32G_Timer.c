/**
 * @file    STC32G_Timer.c
 * @brief   STC32G Timer0~Timer4 驱动实现
 * @author  STCAI / boweny
 * @date    2026-05-31
 *
 * @details
 * 本文件实现 Timer0~Timer4 的基础初始化逻辑，负责把上层配置结构体写入对应定时器寄存器。
 * 当前工程中，Timer0 参与任务节拍，Timer2 与 UART2/GPS 波特率链路存在强耦合，
 * 因此修改定时器用途前必须先核对当前资源占用。
 *
 * @note 当前职责边界：
 * - 本文件只负责硬件定时器初始化；
 * - 周期语义与中断用途由上层模块和 ISR 决定；
 * - 不在此处实现任务调度、GPS 解析或控制逻辑。
 */

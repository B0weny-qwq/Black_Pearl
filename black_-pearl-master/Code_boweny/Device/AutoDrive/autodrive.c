...
static u32 AutoDrive_Abs32(int32 value)
{
    if (value < 0L) {
        return (u32)(-value);
    }
...
static void AutoDrive_SetMotorBack(u16 pwm)
{
    Motor_SetBothSpeed(-(int16)pwm, -(int16)pwm);
}
...
u8 AutoDrive_IsCanActive(const AutoDrive_PointRaw_t *point)
{
    ...
    if (!AutoDrive_PointRawValid(point)) {
        return 0U;
    }
    if (!AutoDrive_PointRawValid(&g_idle_position)) {
        return 0U;
    }
    if (!AutoDrive_GpsReady()) {
        return 0U;
    }
...
void AutoDrive_SetReturnPositionRaw(const u8 *data_m)
{
    ...
    if (!AutoDrive_IsCanActive(&g_return_position)) {
        return;
    }
...
void AutoDrive_SetFishPositionRaw(const u8 *data_m)
{
    ...
    if (!AutoDrive_IsCanActive(&g_fish_position)) {
        return;
    }
...
void AutoDrive_TriggerReturn(void)
{
    if (g_autodrv_cfg.auto_ret_onoff != 0x30U) {
        if (g_autoDrive_fail_flag) {
            return;
        }
        if (g_autoDrive_state != AUTO_DRIVE_IDLE) {
            return;
        }
        if (AutoDrive_IsCanActive(&g_autodrv_cfg.ret_point)) {
...
static double AutoDrive_CalDistanceLon(const AutoDrive_PointRaw_t *now,
                                       const AutoDrive_PointRaw_t *des)
{
    ...
    sec1 = (double)(now->lon_whole % 100U) + ((double)now->lon_frac) / 10000.0;
    sec2 = (double)(des->lon_whole % 100U) + ((double)des->lon_frac) / 10000.0;
    ...
    if (dd1 != 0U) {
        return (double)dd1 * 111130.0 + sec1 * 1850.0;
    }
    return sec1 * 1850.0;
}

static double AutoDrive_CalDistanceLat(const AutoDrive_PointRaw_t *now,
                                       const AutoDrive_PointRaw_t *des)
{
    u16 dd1;
    u16 dd2;
    ...
    sec1 = (double)(now->lat_whole % 100U) + ((double)now->lat_frac) / 10000.0;
    sec2 = (double)(des->lat_whole % 100U) + ((double)des->lat_frac) / 10000.0;
                                       ...
                                       const u8 *despositionData)
{
    ...
    if (distance_width <= 0.0) {
        if (distance_height <= 0.0) {
            return 65535U;
        }
        return 90U;
    }

    angel = atan(distance_height / distance_width) * 180.0 / 3.14159265;
    if (angel > 360.0) {
        return 65535U;
    }
    return (u16)angel;

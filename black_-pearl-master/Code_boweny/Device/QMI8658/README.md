...
|| 默认配置 | Legacy retest: CTRL2=0x07, CTRL3=0x07 |
|| 加速度灵敏度 | 8192 LSB/g (±4G) |
|| 陀螺仪灵敏度 | Legacy retest uses 2048 LSB/°/s assumption |
|| 系统时钟 | Fosc = 24MHz |
...
- QMI8658 is running the stable bring-up register pair:
  `CTRL2=0x07`, `CTRL3=0x07`.
- `QMI8658_CLEAR_DATAPATH_ENABLE=0`, so the extra `CTRL6/CTRL8/FIFO/CTRL9`
  cleanup path is skipped.
- `QMI8658_SOFT_RESET_ENABLE=0`: the driver does not actively write
  `RESET=0xB0` during normal bring-up.
- The replaced QMI8658 restored `RESET_STATE=0x80` and `ready ... acc=...`;
  the previous chip had a dead data/sampling domain while ID and CTRL registers
  were still readable.
- `QMI8658_DIAG_ENABLE=0`, so init no longer reads timestamp/temp/raw before
  `QMI8658_PowerOnSelfTest()` performs the same direct acc read used in the
  original bring-up.
- After repeated `ReadAcc()` failure, `QMI8658_DumpRawRegs()` performs
  single-byte reads of ID, status, temp, acc, and gyro registers to separate
  multi-byte auto-increment issues from a dead data register domain.
...
- 当前运行配置：
  - `CTRL2 = 0x07`，legacy bring-up retest
  - `CTRL3 = 0x07`，legacy bring-up retest
  - `CTRL5 = 0x11`
  - `CTRL7 = 0x03`
- Ready 轮询：`QMI8658_Wait_AccReady()` 只检查 `STATUS0.bit0(aDA)`，避免混用 `STATUSINT`
- 启动初始化会强制软复位一次，显式关闭 FIFO/SyncSample 扩展路径，并输出 `CTRL1/2/3/5/6/7/8/9`、FIFO、`STATUSINT/STATUS0`、timestamp、temp 和首帧 raw 数据；若 200ms 内仍未 ready，会追加约 1 秒短轮询窗口，确认数据是否只是延迟输出
...
读取三轴角速度。旧版 bring-up 复测阶段先按 ±16°/s，灵敏度 2048 LSB/°/s。返回: `0`=成功, `-1`=失败。

...
## [2026-05-23] - v1.7.67 yaw 自稳降速阈值放宽

### 优化改进
- **[手动油门冲顿]** 将 `SHIP_YAW_HOLD_DERATE_START_CD` 从 `300` 调整为 `1000`，即 yaw 自稳基础速度降额由偏航误差 `3.00°` 后才开始改为 `10.00°` 后才开始，避免小角度航向噪声和水面扰动导致推油门时一冲一冲。
- **[降速曲线放缓]** 将 `SHIP_YAW_HOLD_DERATE_FULL_CD` 从 `1000` 调整为 `2000`，即满降额由 `10.00°` 改为 `20.00°`，避免刚超过 10 度就把基础油门压到最低。
- **[配置注释同步]** 同步更新 `User/FeatureSwitch.h` 中降速阈值注释，现场调参时可直接按 `0.01°` 单位核对。

### 开发者备注
- 本次只改 yaw 自稳基础速度降额阈值，不改 PID 增益、不改差速限幅、不改电机 PWM 输出链路。
- 若实船仍有冲顿，优先看 `[CTRL] out/mode` 与 yaw-hold 日志里的 `err/base/df/l/r`，确认是否仍在 `MANUAL_YAW_HOLD` 与 `MANUAL_OPEN_LOOP` 之间来回切换。

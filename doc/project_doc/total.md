...
- `Motor_Stop()` 回到 `50%` 占空比中点，不再把 PWM 拉到 `0%`
- `speed=-1000~+1000` 映射为围绕 `50%` 中点上下偏移
- `PWMA CH3/CH4` 持续接管 `P2.4~P2.7`，停机时不再退回 GPIO 上拉态
- 本轮未改 `Driver/src/STC32G_PWM.c`，修复全部集中在 `Code_boweny/Device/MOTOR/Motor.c`
...
- `sat`：优先使用完整 GSA PRN 计数，GSA 不完整或为 0 时回退 GGA 卫星数，最大限制 24。
- `angle_u16`：RMC 航向角整数度，对齐老版 `nmea41_Get_Angel()`，不是 `deg * 100`。
- `lon1/lon2`：从 RMC 原始经度字符串 `dddmm.mmmm` 直接拆分，例如 `12156.4607500 -> 12156 / 4607`。
- `lat1/lat2`：从 RMC 原始纬度字符串 `ddmm.mmmm` 直接拆分，例如 `3724.2182068 -> 3724 / 2182`。
- `power`：当前电量采样映射字节。
- `auto`：今晚固定保留兼容字段，不恢复自动驾驶副作用。

补充说明：
- 当前 `0x12` 发完后会立即重开工作信道 RX，避免状态回包后出现额外接收空窗。
- `0x12` 方向字节继续保持老版常量 `E/W`，真实半球只在 `gps state` 日志里显示，避免破坏遥控器旧解析。
...
- 路由：UART2 `P1.0/P1.1`
- 用于无线 `0x12` 状态包回传
- 当前 `ShipProtocol_SendGpsOnce()` 除了保留老版 `0x12` 15 字节 payload 外，还会打印正式核对日志：
- `gps state fix=... legacy=... sat=... lon=... lat=... angle=... power=... seq=...`
- `gps sat source gsa=... gga=... report=...`
- `gps payload oldfmt ew=E lon1=... lon2=... ns=W lat1=... lat2=...`
- `gps payload bytes=...`
...
2. 电机方向最终语义
   - 当前 `Motor` 已按半桥驱动 `HIN/LIN#` 实际接法固定 CH3/CH4 极性，并切到 `50%` 中点 + 差分偏移模型
   - 但“前进/左转”与实船推进方向是否完全一致，仍必须以现场水测结果为准
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-05-07 | v1.7.55 | 修正半桥电机 PWM 控制模型：`Motor` 层改为 `50%` 占空比中点静止、速度围绕中点上下偏移；停机时不再撤销 PWM 输出回 GPIO 上拉态；并按板级 `HIN/LIN#` 接法固定 CH3/CH4 极性，同时同步更新 README/total/date 文档。 |
| 2026-05-07 | v1.7.54 | 复核老版 `GPS_DK2_RMC` 与 `RF_Send_Gps_Data()`，修正 `0x12` GPS 坐标为 RMC 原始 `dddmm.mmmm/ddmm.mmmm` 拆分格式，航向角回到整数度，卫星数恢复完整 GSA 优先、否则回退 GGA 的老版规则，并补齐 payload 字节日志与上位机解析。 |
| 2026-05-07 | v1.7.53 | 新增串口日志上位机，支持串口选择、波特率配置、2 万条缓存、GPS/按键/油门解析与本地脚本启动。 |

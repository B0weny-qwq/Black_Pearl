...
- `AHRS_Reset / AHRS_UpdateRaw6Axis / AHRS_UpdateRawMag / AHRS_GetState`
- `AHRS_State_t` 字段不变
- `[AHRS] I: r=... p=... y=... g=... f=...`
- `[AHRS] I: r=... p=... y=... ys=... g=... f=...`
- `[AHRS] I: r=... p=... y=... yr=... f=...`

Keil C51 下由于浮点四元数上下文增加了静态存储占用，本轮还把 AHRS 上下文迁到了 `xdata`，避免 `EDATA` 链接溢出。
...
- `f=17`：已经有姿态输出，但陀螺零偏还没有进入 ready
- `f=1F`：姿态、加速度参考、磁力计有效位、陀螺零偏都已进入 ready
...
1. 编译必须通过，确认 AHRS 路径重新进入链接。
2. 上电静置后 `flags` 应从 `0x17` 进入 `0x1F`。
3. 平放时 `roll/pitch` 应收敛到接近 0，`g=` 三轴应接近 0。
4. `ys` 计满后应出现 `yr=+0.00`。
5. 手动倾斜板子时 `roll/pitch` 方向应正确，不再出现明显欧拉串轴。
6. 缓慢旋转船体时 `yaw` 应连续变化，停止后不会像旧版那样长期单向漂移。

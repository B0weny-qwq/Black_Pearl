...
|------|----|----------|
| 无按键 | `0xA0` | 不动作 |
| E | `0xA1` | 带符号油门 `raw_ud-100 >= +50` 时进入定速巡航；巡航中再按一次 E 退出；巡航中带符号油门 `raw_ud-100 <= -80` 退出；按 E 且 `raw_ud-100 <= -80` 时也会停巡航/停机；收到 E 但油门不够会打印 `cruise-ignore`；同时关闭自动驾驶模式 |
...
- 如果只有 1 号钓点有效，第二次收到同一坐标即可尝试去 1 号，不需要等待 2..5 号。
...
- 钓点列表同样只保存在 RAM 中，重新上电后需要遥控器重新下发。
...
- 再次发送已保存点应看到 `[DATA] ... res=5(start) idx=对应编号`，若距离/GPS 不满足则为 `res=4(reject-distance)`。

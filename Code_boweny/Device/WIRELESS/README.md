...
| `0x13` | `SET_RETURN` | 遥控器 -> 船 | 10 字节点位 | 设置返航原点，坐标有效时写入 EEPROM，并尝试进入返航 |
| `0x14` | `SET_DESTINATION` | 遥控器 -> 船 | 10 字节点位 | 不保存钓点；每次只把当前帧坐标作为本次去点目标 |
| `0x15` | `SWITCH_AUTO_RETURN` | 遥控器 -> 船 | `switch + 可选 10 字节点位` | 更新运行期自动返航开关；携带有效点位时更新并持久化返航原点 |
...
| `0x14` | `autoDrive_Set_FishPosition()` | `AutoDrive_SetFishPositionRaw()` | 10 字节点位格式对齐；船端不保存钓点，只按当前帧坐标尝试去目标点 |
| `0x15` | `autoDrive_Set_Switch()` | `AutoDrive_SetSwitchRaw()` | 完整 11 字节旧遥控器帧兼容；短包只改运行期开关，携带有效点位时才保存返航原点 |
...
| `0x14` | 10 字节点位 | `AutoDrive_SetFishPositionRaw()`，不保存钓点；每个有效坐标只作为本次当前目标尝试进入巡航 |
| `0x15` | `switch + 可选 10 字节点位` | 更新运行期自动返航开关；若长度至少 11 字节且点位有效，同时更新并持久化返航原点；开关不为 `0x30` 时立即尝试返航 |
...
- `0x14` 不再接收或维护 1..5 钓点表，船端没有钓点编号和历史匹配。
- 每次 `0x14` 都只使用遥控器当前发送的 10 字节点位；同一坐标重发就是新的去点请求。
- 到点停车、手动停止、超时失败、模式切换或重新上电后，旧 `0x14` 坐标不会被船端保留。

`0x14` 数据日志写到 `DATA` tag，便于遥控器日志串口过滤：

```text
[DATA] I: 0x14 rx fl=15 pl=10 xor=38/38 nav=5(start)
...
Flash/EEPROM 只保存返航原点。`AutoDriveCfg_Save()` 使用 STC EEPROM/IAP 地址 `0x000600` 写入带 magic/version/checksum 的返航点记录，避开 NorthCalib 的 `0x000200/0x000400` A/B 槽。
...
- `auto_ret_onoff` 只在本次运行 RAM 中生效，重新上电后回到默认 `0x30`。
- 低电、链路超时或 `0x15` 触发返航时，使用当前 RAM 中的返航点；上电时 RAM 返航点来自 EEPROM 中的原点记录。
- `0x14` 钓点不进 RAM 表，也不写 Flash/EEPROM；重新上电后必须由遥控器重新下发当前要去的钓点。
...
[DATA] I: 0x14 rx fl=... pl=10 xor=.../... nav=...(...)
...
- `0x15` 正常完整载荷为 11 字节。
- `0x14` 任意有效点只应看到 `[DATA] ... nav=5(start)` 或 `nav=4(reject-distance)`，不再出现 `save=` 或 `idx=`。
- AutoDrive 已运行时重发 `0x14`，应看到 `nav=0(busy)`，不会覆盖当前目标。
- 到点或手动停止后再次发送同一坐标，应按新的当前帧重新尝试去点，不依赖船端钓点编号。
- 返航原点保存检查 `0x000600` EEPROM 区；不再检查旧 `0x0001F800` flash 区。
...
| `0x14` 钓点鉴别 | 船端不保存钓点；每次只使用遥控器当前下发坐标尝试去点 | 避免上电后历史钓点或重发状态和本地校验冲突 |
...
| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-06-06 | v2.0 | 同步 `0x14` 钓点不保存、每次使用遥控器当前坐标；返航原点改为 EEPROM `0x000600` 持久化，`0x14` DATA 日志移除 `save/idx`。 |

...
- `PID` 是通用定点控制器，不知道 GPS、IMU、磁力计或电机具体含义。
- `ShipControl` 负责把 `MainLoop_GetHeadingDeg100()` 的航向误差归一化后送进 PID。
- `AutoDrive` 和 `NorthCalib` 只提交目标航向或模式请求，不直接操作 PID 内部状态。
...
- `PID` 是通用定点控制器，只维护误差、积分、微分和限幅状态
- 当前工程主要由 `ShipControl.c` 复用到 yaw-hold 控制，`PID` 本身不感知 IMU、GPS、磁力计或电机语义
- 目标值生成、更新周期选择、控制模式切换和安全门限都由上层控制模块负责

## 版本历史

| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-05-31 | v1.1 | 补充当前职责边界，明确 PID 只提供通用定点控制能力，yaw-hold 策略归 ShipControl 管理 |

...
- `0x13/0x14/0x15` 只负责写入返航原点、接收本次钓点目标或更新自动返航运行期开关，不直接下发固定航向角。
- 船端不维护钓点表；新的有效 `0x14` 只更新当前前往目标 `g_fish_position` 并尝试前往，重启、到点或停止后不会保留为本地钓点。
- Flash/EEPROM 只保存返航原点：有效 `0x13` 或完整有效 `0x15` 写入 `AutoDriveCfg_Save()`，当前记录地址为 `0x000600`；`0x14` 钓点不写 RAM 表，也不写 Flash/EEPROM。
